...
function SettingsSidebar({
  ...
  return (
    <aside className="sticky top-0 flex h-screen w-[252px] shrink-0 flex-col overflow-hidden border-r border-slate-200 bg-[#f3f4f6] shadow-[inset_-1px_0_0_rgba(148,163,184,0.14)]">
      <div className="border-b border-slate-200 px-3 py-2.5">
        <button
          type="button"
          onClick={() => navigate('/dashboard')}
          className="flex items-center gap-2 text-[12px] font-medium text-slate-800"
        >
        ...
        <div className="mt-2.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-1.5 py-2">
        <div className="space-y-1">
          <p className="px-4 text-[9px] font-semibold uppercase tracking-[0.14em] text-slate-400">
            Personal settings
          </p>
          <nav className="space-y-0 px-1">
            {personalSettingsItems.map((item) => (
              <button
                key={item}
                type="button"
                onClick={() => onSelectPersonalSetting(item)}
                className={`flex w-full items-center rounded-xl px-2.5 py-1 text-left text-[10px] transition ${
            ...
            {workspaceSectionItems.map((item) => (
              <div key={item} className="space-y-0.5">
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-xl px-2 py-1.5 text-left text-[8px] text-slate-800 transition hover:bg-[#e9edf2] hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.28),0_1px_2px_rgba(15,23,42,0.06)]"
                >
                  <span className="pr-1 leading-4 whitespace-nowrap">{item}</span>
                  ...
                  <div className="space-y-0.5 pl-3.5">
                    {deliverabilitySuiteItems.map((subItem) => (
                      <button
                        ...
                        className={`flex w-full items-center rounded-xl px-2 py-1 text-left text-[9px] transition ${
                  ...
                  <div className="space-y-0.5 pl-3.5">
                    {usersAndTeamsItems.map((subItem) => (
                      <button
                        key={subItem}
                        type="button"
                        className="flex w-full items-center rounded-xl px-2 py-1 text-left text-[9px] text-slate-700 transition hover:bg-[#e9edf2] hover:text-slate-900"
                      >
                        <span className="whitespace-nowrap">{subItem}</span>
                      </button>
                    ))}
                  ...
                  <div className="space-y-1 pl-3.5">
                    {teamConversationsItems.map((subItem) => (
                      <button
                        key={subItem}
                        type="button"
                        className="flex w-full items-center rounded-xl px-2 py-1.5 text-left text-[9px] text-slate-700 transition hover:bg-[#e9edf2] hover:text-slate-900"
                      >
                        {subItem}
                      </button>
                    ))}
      ...
      <div className="border-t border-slate-200 px-3 py-2">
        <button
          type="button"
          className="flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-1.5 text-[11px] font-medium text-slate-800 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition hover:bg-[#e9edf2]"
        >
